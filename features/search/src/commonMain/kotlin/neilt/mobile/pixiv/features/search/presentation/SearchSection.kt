...
import neilt.mobile.pixiv.features.search.presentation.result.ResultView
...
data object PixivSearchSection : Destination {
    ...
    @Serializable
    internal data class ResultScreen(val exploreType: ExploreType, val keyword: String) : Destination
}
...
fun NavGraphBuilder.addPixivSearchSection() {
    navigation<PixivSearchSection>(
        ...
        composable<PixivSearchSection.ResultScreen> {
            val args = it.toRoute<PixivSearchSection.ResultScreen>()
            ResultView(
                exploreType = args.exploreType,
